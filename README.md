...
| Модуль | Описание |
|--------|----------|
| `fuzzy_search.py` | Smart Fuzzy Search с классификацией |
| `ai_search.py` | Интеграция с GPT для AI-поиска |
...
База данных не включена в репозиторий (интеллектуальная собственность). Варианты получения:
1. **Импорт из Excel** — используйте `sync_db_from_excel.py`
2. **Парсинг источников** — запустите парсеры из `parsers/`
3. **Демо-версия** — свяжитесь с автором
...
- База данных не публикуется (интеллектуальная собственность)
