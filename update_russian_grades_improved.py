...
def find_grade_variations(conn, grade):
    """Находит варианты марки в базе данных (без звездочек, с звездочками и т.д.)"""
    cursor = conn.cursor()
    ...
    cursor.execute("SELECT id, grade FROM steel_grades WHERE grade = ?", (grade,))
    ...
    if grade != grade_clean:
        cursor.execute("SELECT id, grade FROM steel_grades WHERE grade = ?", (grade_clean,))
        clean_match = cursor.fetchone()
        if clean_match:
            return clean_match
    
    # Ищем варианты с разными регистрами
    cursor.execute("SELECT id, grade FROM steel_grades WHERE UPPER(TRIM(grade)) = UPPER(?)", (grade_clean,))
...
def update_grades_in_database(grades_map):
    ...
    for latin_grade, cyrillic_grade in grades_map.items():
        # Ищем марку (с учетом вариантов)
        found_row = find_grade_variations(conn, latin_grade)
        
        if found_row:
            grade_id, current_grade = found_row
            ...
            if duplicate_row:
                ...
                if duplicate_id != grade_id:
                    ...
                    print(f"    Пропускаю обновление '{current_grade}' (ID {grade_id})")
                    not_found_count += 1
                    continue
            ...
            try:
                ...
                if cursor.rowcount > 0:
                    updated_count += 1
                    print(f"  OK: Обновлена марка ID {grade_id}: '{current_grade}' -> '{cyrillic_grade}'")
                else:
...
def check_duplicates():
    ...
    if duplicates:
        ...
        for grade, count in duplicates[:20]:  # Показываем первые 20
            cursor.execute("SELECT id, analogues, base FROM steel_grades WHERE grade = ?", (grade,))
            rows = cursor.fetchall()
            print(f"\n  Марка '{grade}' встречается {count} раз(а):")
            for row_id, analogues, base in rows[:3]:  # Показываем первые 3 записи
                analogues_str = (analogues or '')[:50] + '...' if analogues and len(analogues) > 50 else (analogues or '')
                print(f"    ID {row_id}, Base: {base}, Analogues: {analogues_str}")
    else:
...
def main():
    """Основная функция"""
    print("=" * 60)
    print("Обновление русских марок сталей в базе данных (улучшенная версия)")
    print("=" * 60)
    ...
    # Проверяем сколько марок теперь на кириллице (проверяем по наличию кириллических символов)
    ...
    for (grade,) in all_grades:
        if grade and any(char in cyrillic_chars for char in grade):
            cyrillic_count += 1
    conn.close()
    ...
    print(f"  Марок на кириллице: {cyrillic_count}")
