...
async def perform_search(update: Update, grade_name: str, context: ContextTypes.DEFAULT_TYPE = None, force_ai: bool = False):
    ...
    try:
        ...
        if not results:
            ...
            if attempt_count == 1:
                # First attempt - offer choice: correct spelling OR AI search
                keyboard = [
                    [
                        InlineKeyboardButton("✏️ Исправить название", callback_data=f'retry_search:{grade_name}')
                    ],
                    [
                        InlineKeyboardButton("🤖 AI Search (Perplexity)", callback_data=f'confirm_ai:{grade_name}'),
                    ]
                ]
                ...
                await update.message.reply_text(
                    ...
                    f"🤔 **Что делать дальше?**\n\n"
                    f"**Вариант 1: Исправить название марки**\n"
                    f"• Проверьте написание (опечатка, регистр, пробелы)\n"
                    f"• Попробуйте без пробелов (ШХ15 вместо ШХ 15)\n"
                    f"• Проверьте раскладку клавиатуры (латиница/кириллица)\n"
                    f"• Уточните производителя или стандарт\n\n"
                    f"**Вариант 2: Поиск через нейронную сеть (Perplexity AI)**\n"
                ...
                # Second+ attempt - automatic AI search
                await update.message.reply_text(
                    f"🔍 Марка `{grade_name}` не найдена в базе данных.\n\n"
